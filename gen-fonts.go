...
func main() {
	log.SetPrefix("xolonium-gen: ")
	log.SetFlags(0)

	var (
		src = flag.String(
			"src",
			"https://fontlibrary.org/assets/downloads/xolonium/e00c124f3e1b130e5ec2a7eee2f4f1b8/xolonium.zip",
			"remote zip file holding TTF files for Xolonium fonts",
		)
	)

	flag.Parse()

	tmp, err := os.MkdirTemp("", "go-fonts-xolonium-")
	if err != nil {
	...
	case strings.HasPrefix(*src, "http://"),
		...
		if err != nil {
			log.Fatalf("could not fetch Xolonium sources: %+v", err)
		}
	default:
		zr, err = zip.OpenReader(*src)
		if err != nil {
			log.Fatalf("could not open local Xolonium sources: %+v", err)
		}
	}
	defer zr.Close()
...
func fetch(tmp, src string) (*zip.ReadCloser, error) {
	...
	f, err := os.Create(path.Join(tmp, "xonolonium.zip"))
	if err != nil {
...
func do(ttfName string, src []byte) error {
	...
	fmt.Fprintf(b, "// from the Xolonium font family.\n")
	fmt.Fprintf(b, "package %[1]s // import \"github.com/go-fonts/xonolonium/%[1]s\"\n\n", pkgName)
